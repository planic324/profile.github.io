(function($) {
    ...
    $(document).ready(function() {
        ...
        if($('.header').hasClass('v2')) {
            $('.header:not(.noinclude)').include('/html/include/inc_header2.html'); // 스마트A온라인
        }
        else if($('.header').hasClass('v3')) {
            $('.header:not(.noinclude)').include('/html/include/inc_header3.html'); // 스마트A온라인 새창
        }
        else if($('.header').hasClass('v4')) {
            $('.header:not(.noinclude)').include('/html/include/inc_header4.html');
        }
        else if($('.header.v5').hasClass('tedge_header')) {
            $('.header:not(.noinclude)').include('/html/include/inc_header5.html'); // T edge 가입요청확인
        }
        else if($('.header').hasClass('t_header')) {
            $('.header:not(.noinclude)').include('/html/include/inc_header6.html'); // T 메인헤더
        }
        else if($('.header').hasClass('tedge_header')) {
            $('.header:not(.noinclude)').include('/html/include/inc_header7.html'); // T edge 메인헤더
        }
        else if($('.header').hasClass('taxagent')) {
            $('.header:not(.noinclude)').include('/html/include/inc_header8.html'); // 세무대리연결 SAO
        }
        ...
        else if($('.header').hasClass('ledge_header')) {
            $('.header:not(.noinclude)').include('/html/include/inc_wel_header2.html'); // L edge 메인헤더
        }
        else if($('.header').hasClass('  ')) {
            $('.header:not(.noinclude)').include('/html/include/inc_header_white.html'); // FG 동영상가이드 비회원 헤더
        }
        else if($('.header').hasClass('rnd_header')) {
            $('.header:not(.noinclude)').include('/html/include/inc_header11.html'); // R&D전문플랫폼(범부처협업클라우드) 헤더
        }
        else if($('.header').hasClass('evg_header')) {
            $('.header:not(.noinclude)').include('/html/include/inc_header_evg.html'); // EVG 법인조정 동영상가이드(삭제될 수 있음)
            $('#dzfont_btn').css('display','none');
            $('#buildtest_btn').css('display','none');
        }
        else if($('.header').hasClass('dt_header_white')) {
            $('.header:not(.noinclude)').include('/html/include/inc_dt_header_white.html'); // DT올인원팩 검색화면 헤더
        }
        else if($('.header').hasClass('dt_header_white2')) {
            $('.header:not(.noinclude)').include('/html/include/inc_dt_header_white2.html'); // DT올인원팩 검색화면 헤더
        }
        else if($('.header').hasClass('darkmode')) {
            $('.header:not(.noinclude)').include('/html/include/inc_header_dark.html'); // 다크모드 헤더
        }
        else if($('.header').hasClass('darkmode2')) {
            $('.header:not(.noinclude)').include('/html/include/inc_header_dark2.html'); // 다크모드 비회원 헤더
        }
        else if($('.header').hasClass('wehagov_fg')) {
            $('.header:not(.noinclude)').include('/html/include/inc_header_fg.html'); // 다크모드 비회원 헤더
        }
        else if($('.header').hasClass('dark_with_banner')) {
            $('.header:not(.noinclude)').include('/html/include/inc_header_dark_banner.html'); // 다크모드+배너 헤더
        }
        else if($('.header').hasClass('non_members')) {
            $('.header:not(.noinclude)').include('/html/include/inc_header_non_members.html'); // 비회원 헤더
        }
        else if($('.header').hasClass('bn_header')) {
            $('.header:not(.noinclude)').include('/html/include/inc_bn_header.html'); // 디지털마케팅플랫폼 헤더
        }
        else if($('.header').hasClass('smp_header')) {
            $('.header:not(.noinclude)').include('/html/include/inc_smp_header.html'); // 의료플랫폼 헤더
        }
        else if($('.header').hasClass('wehago_lite_header')) {
            $('.header:not(.noinclude)').include('/html/include/inc_wehago_lite_header.html'); // WEHAGO LITE 헤더
        }
        else if($('.header').parents('#wrap').hasClass('lulu-ui')) {
            $('.header:not(.noinclude)').include('/lulu-ui/src/cfp/include/inc_cfp_header.html'); // 초록우산어린이재단 헤더
        }
        else {
            $('.header:not(.noinclude)').include('/html/include/inc_header.html');
        }
        $('.sm .header:not(.noinclude)').include('/html/include/inc_header_market.html');
        $('.sm .header_v2:not(.noinclude)').include('/html/include/inc_header_market_v2.html');
        $('.sm .header_v3:not(.noinclude)').include('/html/include/inc_header_market_v3.html');
